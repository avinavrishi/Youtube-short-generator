...
class TextQuizRenderer(BaseRenderer):
    ...
    def build_video(self, video_id, topic, questions, bg_type, music_dir, images_dir, videos_dir, fonts_dir, voiceovers_dir, output_dir, tts_voice, is_preview=False, template="classic", thumbnail_path=None, selected_char=None, generate_script=True):
        try:
            ...
            for idx, q_data in enumerate(questions):
                # Robust parsing for Question Data
            ...
            for idx, asset in enumerate(q_assets):
                ...
                if os.path.exists(bing_path):
                    audio_mixes.append(f"[{get_input_idx(bing_path)}:a]volume=0.5,adelay={a_at}|{a_at}[a_bing{idx}]")
            ...
            out_path = os.path.join(out_dir_final, f"Quiz_{video_id}.mp4")
